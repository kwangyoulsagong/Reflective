...
export const useHeaderIDs = (
  ...
  }, [contents, processHeaders]); // contents나 contentRef가 변경될 때마다 실행
};
...
export const useToC = (
  ...
  const updateCirclePosition = useCallback(
    ...
    [setCirclePosition, setFilledHeight]
  );
  ...
  const createTableOfContents = useCallback(() => {
    if (!contentRef.current) return;
  ...
  }, [contentRef, updateCirclePosition]);
  useEffect(() => {
    if (!contents) return;

    // DOM이 업데이트될 때까지 기다린 후 목차 생성
    const timer = setTimeout(() => {
  ...
  }, [contents, createTableOfContents, handleScroll]);
};
